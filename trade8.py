...
stock_symbol = "AAPL"  # Change to "MSFT", "GOOGL", etc.
...
X = X[valid_indices]
y = y[valid_indices]
print(f"Sequence buy percentage: {np.mean(y) * 100:.2f}%")

# Debug: Check sample labels
sample_indices = range(15, 25)  # Check a few early samples
print("Sample labels vs. returns:")
for i in sample_indices:
    if i < len(dates) - sequence_length:
        ret = (raw_prices[i + sequence_length] - raw_prices[i]) / raw_prices[i] * 100
