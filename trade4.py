...
for i in range(len(full_data) - sequence_length * 2):
    ...
    y.append(1 if future_return > 0.03 else 0)  # Threshold at 3%
...
print("Sample y:", y[:10])
print("Sample raw prices (current vs. 15 days later):", [(dates[i], raw_prices[i], dates[i+sequence_length], raw_prices[i+sequence_length], (raw_prices[i+sequence_length] - raw_prices[i]) / raw_prices[i] * 100) for i in range(15, min(25, len(raw_prices) - sequence_length))])
...
for signal, price, future in zip(lstm_signals, test_prices, future_prices):
    ...
    if signal == "buy":
        lstm_profits.append(price * actual_return)  # Full return for buy
    else:
